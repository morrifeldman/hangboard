...
export function ImportClimbsModal({ isOpen, onClose, onImportComplete }: Props) {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type === "text/csv") {
      ...
      try {
        ...
        setStatus(`Saving ${climbs.length} climbs…`);
  ...
  const handleUrlImport = async () => {
    ...
    setIsImporting(true);
    try {
      setStatus("Fetching from Mountain Project…");
        ...
        <div className="p-6">
              ...
              <>
                ...
                <div>
                  ...
                  <button
                    ...
                    disabled={!csvUrl.trim() || isImporting}
                    className="w-full mt-2 bg-green-600 text-white py-2 px-4 rounded-xl hover:bg-green-700 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                  >
                    {isImporting && <Spinner />}
              ...
              <>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-gray-700 file:text-gray-300 hover:file:bg-gray-600 file:cursor-pointer cursor-pointer border-2 border-dashed border-gray-600 rounded-lg p-3 disabled:opacity-50 disabled:cursor-not-allowed"
                  />
                </div>
              </>
            )}
          ...
          <div className="flex gap-3 mt-6">
            <button
              ...
              disabled={isImporting}
              className="flex-1 bg-gray-700 text-gray-300 py-2 px-4 rounded-xl hover:bg-gray-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Cancel
            </button>
          </div>
